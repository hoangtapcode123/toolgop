...
def {_join}(herlys,*k):
    ...
    for globals()['hoang_'] in globals()['herlys']:
        if not {__exx}:globals()['hoang_'] += (lambda : '')()
        hoang += {_str}(hoang_);f = {obfint(True)}
    return hoang
def H2SbF7(x):
    return {_int}(x-0x7777)
def c2h6(e):
    br = bytearray(e[len(b"hoang/"):])
    r = 0
    for b in br:
        r = r * 256 + b
    return r
...
CHẾ ĐỘ 1: OBF THẤP (DỄ DEBUG)
CHẾ ĐỘ 2: TRUNG BÌNH (KHUYẾN KHÍCH NÊN DÙNG)
CHẾ ĐỘ 3: CAO (FILE MB CAO)

"""

banner(sokey)
print("Tool lấy trên mạng nên không biết nguồn")
print(text)

def stage(text: str, symbol: str = 'HOANG', col1 = light, col2 = None) -> str:
    if col2 is None:
        col2 = light if symbol == 'HOANG' else purple
...
((
    ((([["HOANG"],
    ["https://drive.google.com/file/d/145jn4108aFSTNzSWV94drmOpcFYadhaj/view?usp=drivesdk"],
    ["PYTHON"],
    3.11
    ],
...
"""
open("encherlyswar-" + _file, "w", encoding="utf8").write(str(code))
print(" Save in", "encherlyswar-" + _file)
